---
order: 10
id: sumin-hong
name: Sumin Hong
group: students     # pi / students / alumni
role: TD Team

photo_by_term:
  "2026-spring": /assets/img/members/2026-spring/홍수민.jpg
terms:
  - "2026-spring"

education:
  - MS / Graduate School of Semiconductor Materials and Devices Engineering, <strong>UNIST</strong> (Mar 2024 – present)
  - BS / Department of Nanotechnology & Advanced Materials Engineering, <strong>Sejong University</strong> (Mar 2019 – Feb 2023)

research:
  - area: Downscaled Top-Gate 2D FETs
    topics:
      - "High-quality dielectric-channel interface"
      - "Low-contact resistance"
      - "Gate-all-around (GAA) FET process"

email: suminhong@unist.ac.kr

# Optional
# links:
#   - Google Scholar: "https://scholar.google.com/citations?user=XXXX"
#   - LinkedIn: "https://www.linkedin.com/in/xxxxx"
---